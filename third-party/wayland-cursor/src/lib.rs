...
use std::sync::Arc;
// `SystemTime`/`UNIX_EPOCH`, `rustix::fs::Mode`, and `rustix::shm` are only used
// by the `shm_open` fallback in `create_shm_fd`, which is compiled out on
// Android. rustix gates its `shm` module out on `target_os = "android"`, so
// importing it unconditionally broke the Termux/Android build (see ADR-008).
...
fn create_shm_fd() -> IoResult<OwnedFd> {
    ...
    // On Android `memfd_create` (above) is the only path: bionic does not
    // provide POSIX `shm_open`/`shm_unlink`, and rustix gates its `shm` module
    // out on `target_os = "android"`. memfd has been available on Android for
    // years (and Termux always provides it), so this is effectively
    // unreachable; if memfd is somehow missing we surface an error rather than
    // referencing the unavailable `shm` module.
    #[cfg(target_os = "android")]
    return Err(IoError::new(
        std::io::ErrorKind::Unsupported,
        "wayland-cursor: memfd_create is unavailable and shm_open is not supported on Android",
    ));

    // Fallback to using shm_open on platforms that provide it.
    #[cfg(not(target_os = "android"))]
    {
