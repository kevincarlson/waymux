...
use rustix::io::Errno;
// `rustix::shm` (and the `Mode`/`ShmOFlags`/`SystemTime` helpers it needs) is
// only used by the `shm_open` fallback in `create_shm_fd`, which is compiled out
// on Android. rustix gates its `shm` module out on `target_os = "android"`, so
// importing it unconditionally breaks the Termux/Android build (see ADR-008).
#[cfg(not(target_os = "android"))]
use rustix::shm::{Mode, ShmOFlags};
use std::{
    fs::File,
    io,
...
impl RawPool {
    fn create_shm_fd() -> io::Result<OwnedFd> {
        ...
        // provide POSIX `shm_open`/`shm_unlink`, and rustix gates its `shm`
        // module out on `target_os = "android"`. memfd is available on Android
        // (and Termux), so this is effectively unreachable; if it is somehow
        // missing we surface an error rather than referencing the unavailable
        // `shm` module.
        #[cfg(target_os = "android")]
        return Err(io::Error::new(
            io::ErrorKind::Unsupported,
            "smithay-client-toolkit: memfd_create is unavailable and shm_open is not supported on Android",
        ));
